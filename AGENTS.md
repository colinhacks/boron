...
**Read [wiki/architecture.md](wiki/architecture.md) first.** It is the map of the system — the four layers and which way they depend, how a paste becomes pixels, the invariants that are load-bearing (above all: everything Boron draws has to be expressible as an ANSI escape sequence), the three renderers that have to agree, and a "where do I change X?" table. This file covers *process*; that one covers *the code*. Share links **are** shipped, and there are two of them: the address bar tracks the workspace as you edit it, and "Copy link" in the export button makes one on demand that can be far longer. Both are written by [src/share.ts](src/share.ts) over the frozen wire format in [src/wire.ts](src/wire.ts). [wiki/share-links.md](wiki/share-links.md) explains why the *first* attempt was pulled, what the seam had to be before a second one could stand, and why the two writers differ, and [wiki/og-images.md](wiki/og-images.md) sketches the server-side card that depended on them. [wiki/fonts.md](wiki/fonts.md) covers the bundled typeface — Nerd-Font-patched JetBrains Mono, split into a text family that always loads and an icon family that only loads when a document contains one — and the measurements that split rests on.
...
There are therefore two parsers, and `parseClipboard` in `paste.ts` tries HTML first: it is the flavour every terminal that writes rich text at all will offer, and on macOS the two say the same thing because the HTML is made out of the RTF. A test pins that — both parsers must produce the same document, character for character, from the same copy. RTF is what is left when nothing made that conversion for us.

**A web page is a source too, and Chrome writes it differently.** Copying out of a browser goes through Blink's own serializer, which inlines *computed* styles onto every element — so a Shiki code block arrives as a flat run of sibling `<span>`s that each restate the block's `background-color`, with no wrapper and no row to read a default off. `uniformBackground` is what keeps that backdrop off every character. Chrome also omits `display`, which is what `relineFromPlainText` is for.
