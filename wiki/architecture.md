...
- **A share URL.** `encodeWorkspace` writes a `SharePayload` (`{ v: 1, doc, theme, bg, frame }`) as JSON, deflates it with `CompressionStream("deflate-raw")` when available, and base64url-encodes it behind a one-character flag: `z` for deflated, `u` for plain. `buildShareUrl` puts it in the fragment (`#s=…`), so the payload never reaches a server log or a referrer header; `shareParamFrom` reads the query string too, because that is the half of a URL that survives being passed through a tool handling only one. Every setting is written in full and never diffed against defaults — a link is a promise about an image, and deflate makes the repetition close to free. The payload also accepts `ansi` instead of `doc`: raw terminal output, parsed on arrival exactly as a paste would be, so a link is constructible without knowing the Slate schema.
...
Everything arriving from outside goes through the sanitizers — `sanitizeFrame` (clamped wider than the sliders, because a drag can exceed them, but still clamped), `sanitizeDocument`, `sanitizeThemeId`, `sanitizeBackgroundId`. `sanitizeBackgroundId` keeps `TRANSPARENT_ID` and turns anything else unrecognized into the default, because passing an unknown id through would quietly hand back a transparent image.
...
- [src/core/serialize.test.ts](../src/core/serialize.test.ts) — SGR emission, the ANSI appearance round-trip across six documents × two themes, and theme independence.
...
- [src/workspace.test.ts](../src/workspace.test.ts) — encode/decode round-trip, that compression shortens rather than grows, the URL-safe alphabet, uncompressed payloads, `ansi`-instead-of-`doc`, per-field sanitizer fallbacks, and fragment-versus-query reading.
